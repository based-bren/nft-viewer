...
export default function DripNftViewer() {
  ...
  const [useWhiteHam, setUseWhiteHam] = useState(false)
  const [useRedHam, setUseRedHam] = useState(false)
      ...
      if (traitType === 'Pants') {
        ...
        if (!usePumpkin) { // Only draw Special if Pumpkin is not active (allow with Onesie)
          ...
          if (traitValue) {
            ...
            if (useSheet && ['Cigar', 'Cigarette', 'Vape'].includes(traitValue)) {
              continue
            }
            await drawImage(ctx, `/traits/${traitType}/${traitValue}.png`)
          }
  ...
  const toggleHamHat = () => {
    ...
    if (useHamHat) {
      setUsePumpkin(false); // Turn off Pumpkin when Ham Hat is activated
    }
  }
  ...
  const toggleWhiteHam = () => {
    setUseWhiteHam(!useWhiteHam)
    if (useWhiteHam) {
      setUseRedHam(false)
      setUseSheet(false)
    }
  ...
  const FeatureButton = ({ isActive, onClick, imageSrc, alt }: { isActive: boolean; onClick: () => void; imageSrc: string; alt: string }) => (
    <button
      className={`w-8 h-8 sm:w-10 sm:h-10 border-2 ${isActive ? 'border-white' : 'border-transparent'} relative flex items-center justify-center bg-black bg-opacity-50 overflow-hidden`}
      onClick={onClick}
    >
      <Image src={`/images/${imageSrc}`} alt={alt} width={32} height={32} className="w-full h-full object-cover" />
      <span className="absolute inset-0 bg-white opacity-0 hover:opacity-20 transition-opacity duration-200"></span>
    </button>
  );
          ...
          <Card className="mb-8 bg-black bg-opacity-50 neon-border rounded-lg">
            ...
            <CardContent className="flex flex-col items-center">
              ...
              <div className="flex justify-center flex-wrap gap-1 sm:gap-2 mb-4">
                ...
                />
                <FeatureButton
                  isActive={useWhiteHam}
                  onClick={toggleWhiteHam}
                  imageSrc="Ham1Button.png"
                  alt="White Ham"
                />
                <FeatureButton
                  isActive={useRedHam}
              ...
              <Button 
                className="bg-green-600 hover:bg-green-500 text-white neon-border rounded w-full"
                onClick={downloadPng}
              >
                Download PNG
              </Button>
